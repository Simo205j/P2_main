...
document.addEventListener("DOMContentLoaded", function () {
  ...
  expandFormButton.addEventListener("click", () => {
    ...
    if (expandFormButton.value === "Close form") {
      ...
      listDOMContainer.style.display = "none"; // hide list container
    }
  });
});
...
function handleServerSentEvent(event) {
  ...
  //4. CREATES NEW TASK ELEMENTS
  handleTaskCreation(sortedTasks);
}
...
function handleTaskCreation(tasks) {
...
function createTaskContainer(task) {
  ...
  const editButton = createEditButton(listElement, task);
...
function createEditButton(taskContainer, task) {
